...
isDrunkDriving = function (value) {
  var normalized;
  if (value == null || value === "") {
    return false;
  }
  normalized = value.toString().trim();
  if (["4", "5", "6", "7", "8"].indexOf(normalized) !== -1) {
    return true;
  }
  return false;
};
setCircle = function (it) {
  return it
    ...
    .style({
      fill: function (it) {
        if (highlightDrunkDriving && it.isDrunkDriving) {
          return colorDrunk;
        }
...
d3.tsv("./accidentXY_113.tsv", function (err, tsvBody) {
  ...
  tsvBody.filter(function (d) {
    ...
    d.isDrunkDriving = isDrunkDriving(d["飲酒情形"]);
  ...
  dc.renderAll();
  updateGraph();
  d3.select("#toggleDrunkHighlight").on("change", function () {
    highlightDrunkDriving = this.checked;
    return gPrints.selectAll("circle").call(setCircle);
  });
  navls = [
    {
      ttl: "事故交叉篩選",
