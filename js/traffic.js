...
d3.tsv("./accidentXY_113.tsv", function (err, tsvBody) {
  ...
  tsvBody.filter(function (d) {
    ...
    d.drinkingCode = +d["飲酒情形"];
    d.isDrunk = d.drinkingCode >= 4 && d.drinkingCode <= 8;
  ...
  dataTable
    ...
    .order(d3.descending)
    .showGroups(false);
  d3.select("#DrunkToggle").on("change", function () {
    highlightDrunk = this.checked;
    return updateGraph();
  });
  dc.renderAll();
  updateGraph();
